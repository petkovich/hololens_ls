...
Uwb::Uwb(const std::string &agent_id, const std::string &world_frame) {
    ...
    this->human_pose_sub =
        n.subscribe("/human_pose", 1000, &Uwb::humanCallback, this);
    this->get_path_srv =
        n.advertiseService("/human_path_srv", &Uwb::return_path_srv, this);
    this->get_pose_srv =
        n.advertiseService("/close_robot_srv", &Uwb::return_pose_srv, this);
...
bool Uwb::return_path_srv(hololens_ls::GetHumanPath::Request &req,
...
bool Uwb::return_pose_srv(hololens_ls::GetRobotPose::Request &req,
...
void Uwb::getPositions() {
    ...
    for (auto it = poses.begin(); it != poses.end(); ++it) {
        // std::cout<< "Robot " << it->id << ": " << it->pose_stamped.pose.x <<
        // ' ' << it->pose_stamped.pose.y << '\n';
