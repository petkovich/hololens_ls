...
int main(int argc, char **argv) {
    ...
    if (!private_nh.getParam("config_filename", config_filename)) {
        ROS_ERROR_STREAM("Parameter "
                         << private_nh.getNamespace()
                         << "/config_filename must be set, exiting");
    ...
    if (!private_nh.getParam("world_frame", world_frame)) {
        ROS_ERROR_STREAM("Parameter " << private_nh.getNamespace()
                                      << "/world_frame must be set, exiting");
        std::exit(EXIT_FAILURE);
    }

    std::ifstream ifs(config_filename);
    if (!ifs.good()) {
        ROS_ERROR_STREAM("Config file " << config_filename
                                        << " can not be found/opened");
        std::exit(EXIT_FAILURE);
    }
